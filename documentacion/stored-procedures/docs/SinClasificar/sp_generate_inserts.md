...
| `modify_date` en BD al extraer | `2019-08-02 15:08:04` |
| Archivo fuente | `../../sql/euroamer_admin_2008.sp_generate_inserts.sql` |
...
|---|---|---|---|---|
| `@table_name` | `varchar` | IN | | |
| `@target_table` | `varchar` | IN | | |
| `@include_column_list` | `bit` | IN | | |
| `@from` | `varchar` | IN | | |
| `@include_timestamp` | `bit` | IN | | |
| `@debug_mode` | `bit` | IN | | |
| `@owner` | `varchar` | IN | | |
| `@ommit_images` | `bit` | IN | | |
| `@ommit_identity` | `bit` | IN | | |
| `@top` | `int` | IN | | |
| `@cols_to_include` | `varchar` | IN | | |
| `@cols_to_exclude` | `varchar` | IN | | |
| `@disable_constraints` | `bit` | IN | | |
| `@ommit_computed_cols` | `bit` | IN | | |

## Tablas / vistas involucradas

| Objeto | Lectura | Escritura | Notas |
|---|---|---|---|
| `existing` | ? | ? | _(auto-detectado — verificar contra el .sql)_ |
| `flot` | ? | ? | _(auto-detectado — verificar contra el .sql)_ |
| `INFORMATION_SCHEMA.COLUMNS` | ? | ? | _(auto-detectado — verificar contra el .sql)_ |
| `INFORMATION_SCHEMA.TABLES` | ? | ? | _(auto-detectado — verificar contra el .sql)_ |
| `or` | ? | ? | _(auto-detectado — verificar contra el .sql)_ |
| `the` | ? | ? | _(auto-detectado — verificar contra el .sql)_ |
| `titles` | ? | ? | _(auto-detectado — verificar contra el .sql)_ |
| `varchar` | ? | ? | _(auto-detectado — verificar contra el .sql)_ |
