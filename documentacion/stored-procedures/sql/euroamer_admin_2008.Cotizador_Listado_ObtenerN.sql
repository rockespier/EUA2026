-- Objeto: euroamer_admin_2008.Cotizador_Listado_ObtenerN
-- Creado en BD: 2019-04-02 15:19:50
-- Modificado en BD: 2019-09-10 10:05:12
...
CREATE PROCEDURE Cotizador_Listado_ObtenerN 
	@pOrigen int,
	@pDestino int,
	@pDias int,
...
BEGIN
	...
	select p.productoID, p.productonombre, productoImporteTarifaFija, p.productoordenlistado, p.productoImporteDiaAdicional,
	     PromocionNombre,PromocionClienteCntPagan,PromocionClienteCntIngresan,PromocionDescuento, productonumerodias 
	--from [PRODUCTO_DESTINO] pd,
	...
	FETCH cursor_tarifa INTO	@vProductoId, @vProductoNombre, @vproductoImporteTarifaFija, @vproductoordenlistado, @vproductoImporteDiaAdicional,
	                            @vPromocionNombre, @vPromocionClienteCntPagan,@vPromocionClienteCntIngresan,@vPromocionDescuento, @vCantidadDiasProd
	WHILE @@FETCH_STATUS = 0
		...
		Begin
			if @vCantidadDiasProd = 365 
			BEGIN
				SET @pDias = @vCantidadDiasProd
			END
		...
		insert into #Resultados 
					...
					@vProductoNombre,					
					@vTotalPasajeros,
					@vproductoordenlistado, @r1,@vPromocionNombre,@vCantidadDiasProd)
