-- Objeto: euroamer_admin_2008.Pasajero_Obtener
-- Creado en BD: 2021-12-12 06:20:57
-- Modificado en BD: 2025-08-14 02:59:29
...
CREATE PROCEDURE [euroamer_admin_2008].[Pasajero_Obtener]	
...
BEGIN
    ...
    begin
			...
			(SELECT valorTipoNombre
			 FROM VALORES_TIPO WHERE valorTipoColumnaTabla='ventaClienteDocumentoTipoId'
			                     AND valorTipoId=PasajeroDocumentoTipoId) as pasajeroDocumentoTipoNombre,
				...
				contactoAgencia,
				CONVERT(VARCHAR(10), CAST(CONVERT(DATETIME, fechasalida, 103) AS DATE), 120) fechaInicio,
				CONVERT(VARCHAR(10), CAST(CONVERT(DATETIME, fecharegreso, 103) AS DATE), 120) fechaFin,
				dia dias
			from PASAJERO			
    ...
    ELSE
	begin
            SELECT top 1 ventaClienteDocumentoTipoId PasajeroDocumentoTipoId, --0
				(SELECT valorTipoNombre
				 FROM VALORES_TIPO WHERE valorTipoColumnaTabla='ventaClienteDocumentoTipoId'
				...
				0	dias --25
		FROM	VENTA
		WHERE	ventaClienteDocumentoTipoId = @pPasajero_DocumentoTipo AND ventaClienteDocumentoNumero = @pPasajero_DocumentoNumero
