-- Objeto: euroamer_admin_2008.VentaMasiva_ProcesarConsultarNuevo
-- Creado en BD: 2019-09-11 06:57:45
-- Modificado en BD: 2025-01-02 08:57:20
...
CREATE PROCEDURE [VentaMasiva_ProcesarConsultarNuevo]
	@pVENTA_MasivoId INT
AS
BEGIN
	...
	DECLARE @vVENTA_Clientenacionalidad VARCHAR(50)
	...
	DECLARE cursor_ventas_masivas CURSOR FOR
		SELECT	ventaIdentity, ventaFechaVigenciaInicio, ventaFechaVigenciaFin, ventaNumeroDias, ventaDestino,
				...
				ventaContactoEmail, ventaContactoTelefono, ventaContactoDistrito, ventaContactoPais, ventaImporteVenta,
	...
	OPEN cursor_ventas_masivas;
		FETCH	cursor_ventas_masivas INTO
				...
				@vVENTA_ContactoEmail, @vVENTA_ContactoTelefono, @vVENTA_ContactoDistrito, @vVENTA_ContactoPais, @vVENTA_ImporteVenta,
				@vVENTA_Clientenacionalidad

	WHILE @@FETCH_STATUS = 0
		BEGIN
			SET @vVENTA_Id = (SELECT correlativoUltimoGenerado+1 FROM CORRELATIVOS WHERE correlativoColumna = 'ventaId')
			...
			WHERE correlativoColumna = 'ventaId'
