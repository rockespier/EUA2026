namespace FrontAssistanceTravelers.WebTravel.Models.Procesos
{
    public class BEVenta
    {
        ...
        // ✅ AGREGADO: Propiedad para devolver la fecha como string formateado
        public string? ventaCobranzaPagoFechaString
        {
            get
            {
                // Si la fecha es nula o es una fecha por defecto, retornar string vacío
                if (ventaCobranzaPagoFecha == DateTime.MinValue ||
                    ventaCobranzaPagoFecha.Year <= 1970)
                {
                    return string.Empty;
                }
                // Retornar la fecha en formato ISO 8601 (YYYY-MM-DD)
                return ventaCobranzaPagoFecha.ToString("yyyy-MM-dd");
            }
        }
    }
}
