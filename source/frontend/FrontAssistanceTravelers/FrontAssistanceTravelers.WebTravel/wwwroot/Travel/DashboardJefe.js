...
async function cambiarValoresChart() {
    ...
    if (permisosVisualizar !== undefined) { 
        ...
        for (let i = 0; i < permisosVisualizar.length; i++) {
            ...
            const NombreMenu = permisosVisualizar[i].menuNombre;
            ...
            if (tipoChart === "1") {
                await getObetenerPie(valorItem, metodoChart, idChart, reporteChart, BusquedaPromotor, dtfechaVigINI, dtfechaVigFIN, tituloCategoria, tituloValor, NombreMenu);
            } else {
                await getObetenerBarras(valorItem, metodoChart, idChart, reporteChart, BusquedaPromotor, dtfechaVigINI, dtfechaVigFIN, tituloCategoria, tituloValor, NombreMenu);
            }
...
function DivNoVisible() {
    $('#chart_DashboardGraficoObtener_75').hide();
    $('#chart_DashboardGraficoObtener_76').hide();
    $('#chart_DashboardGraficoObtener_77').hide();
    $('#chart_DashboardGraficoObtener_78').hide();
    $('#chart_DashboardGraficoObtener_79').hide();    
    $('#chart_DashboardGraficoObtener_80').hide();
}
...
async function getMenuDashBoradJefe() {
    ...
    const urlParametro = "?pPerfilId=" + menuPerfilId + "&pMenuPadreId=" + menuidPageDashboard;
            ...
            const opcionesOTXTipo = {
                ...
                chart: {
                    ...
                    toolbar: {
                        show: true,
                        export: {
                            csv: { filename: 'datos_chart' },
                            png: { filename: 'imagen_chart' }
                        }
                    },
                    dropShadow: {
                        enabled: true,
