...
async function drawTable(dt) {
    ...
    if (BusquedaTipoReporte == 2) {
        ...
        dTable.columns(3).header().to$().text('Importe');
    }
...
function registrarEventosExportacion() {
    ...
    $("#btnExportPdf").on('click', function (e) {
        ...
        exportarReporteTabla('dtVenta', 'pdf', 'Ventas_por_Pais', anioFiltro);
    });
    $("#btnExportPdf2").on('click', function (e) {
        e.preventDefault();
        const anioFiltro2 = (($("#mdvenselAnioSearch option:selected").val() || new Date().getFullYear()) - 1);
        exportarReporteTabla('dtVenta2', 'pdf', 'Ventas_por_Pais', anioFiltro2);
    });
    $("#btnExportAmbosExcel").on('click', function (e) {
        e.preventDefault();
        const anioFiltro = $("#mdvenselAnioSearch option:selected").val() || new Date().getFullYear();
        exportarReporteAmbosCuadros('dtVenta', 'dtVenta2', 'excel', 'Ventas_por_Pais', anioFiltro, anioFiltro - 1);
    });
    $("#btnExportAmbosPdf").on('click', function (e) {
        e.preventDefault();
        const anioFiltro = $("#mdvenselAnioSearch option:selected").val() || new Date().getFullYear();
        exportarReporteAmbosCuadros('dtVenta', 'dtVenta2', 'pdf', 'Ventas_por_Pais', anioFiltro, anioFiltro - 1);
    });
...
async function CargarTodo(dt, periodo) {
    ...
    const listadoVentas = await getVentasPais(BusquedaAnio, BusquedaCodPais, BusquedaTipoReporte, BusquedaCodPromotor, BusquedaMes);
...
async function getVentasPais(int_pAnio, int_pPaisId, int_TipoReporte, int_pPromotorId, int_pMesId) {
    const urlApiFecht = menuUrlApi + "Reportes/VentasPaisObtener";
    const urlParametro = "?int_pAnio=" + int_pAnio + "&int_pPaisId=" + int_pPaisId + "&int_TipoReporte=" + int_TipoReporte + "&int_pPromotorId=" + int_pPromotorId + "&int_pMesId=" + int_pMesId;
