...
async function cargarAcciones() {
    ...
    setTimeout(async () => {
        ...
        // Actualizar títulos con los años
...
function registrarEventosExportacion() {
        ...
        try {
            ...
            const buttons = new $.fn.dataTable.Buttons(table, {
                buttons: [
                    {
                        extend: 'excelHtml5',
                        title: 'Ventas_por_Pais_' + anioFiltro,
                        filename: 'Ventas_por_Pais_' + anioFiltro,
                        exportOptions: {
                            columns: [0, 1],
        ...
        try {
            ...
            const buttons2 = new $.fn.dataTable.Buttons(table2, {
                buttons: [
                    {
                        extend: 'excelHtml5',
                        title: 'Ventas_por_Pais_' + anioFiltro2,
                        filename: 'Ventas_por_Pais_' + anioFiltro2,
                        exportOptions: {
                            columns: [0, 1],
                            footer: false,
                            format: {
    ...
    $("#btnExportPdf").on('click', function (e) {
        ...
        exportarReporteTabla('dtVenta', 'pdf', 'Ventas_Anual_por_Pais', anioFiltro);
    });
    $("#btnExportPdf2").on('click', function (e) {
        e.preventDefault();
        const anioFiltro2 = (($("#mdvenselAnioSearch option:selected").val() || new Date().getFullYear()) - 1);
        exportarReporteTabla('dtVenta2', 'pdf', 'Ventas_Anual_por_Pais', anioFiltro2);
    });
    $("#btnExportAmbosExcel").on('click', function (e) {
        e.preventDefault();
        const anioFiltro = $("#mdvenselAnioSearch option:selected").val() || new Date().getFullYear();
        exportarReporteAmbosCuadros('dtVenta', 'dtVenta2', 'excel', 'Ventas_Anual_por_Pais', anioFiltro, anioFiltro - 1);
    });
    $("#btnExportAmbosPdf").on('click', function (e) {
        e.preventDefault();
        const anioFiltro = $("#mdvenselAnioSearch option:selected").val() || new Date().getFullYear();
        exportarReporteAmbosCuadros('dtVenta', 'dtVenta2', 'pdf', 'Ventas_Anual_por_Pais', anioFiltro, anioFiltro - 1);
    });
...
async function CargarTodo(dt, periodo) {
    ...
    tablaGrid = $("#" + dt).DataTable({
        ...
        "aoColumns": [
            ...
            {
                "mData": "tipoReporte",
                "visible": false  // Ocultar la columna pero mantenerla en los datos
            }
        ],
        "language": {
            "url": sUrlIdioma
    ...
    const listadoVentas = await getVentasPais(BusquedaAnio, BusquedaTipoReporte);
...
async function getVentasPais(int_pAnio, int_TipoReporte) {
    const urlApiFecht = menuUrlApi + "Reportes/VentasPaisAnualObtener";
    const urlParametro = "?int_pAnio=" + int_pAnio + "&int_TipoReporte=" + int_TipoReporte;
