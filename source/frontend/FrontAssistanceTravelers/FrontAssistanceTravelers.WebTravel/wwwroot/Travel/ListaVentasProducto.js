...
function registrarEventosExportacion() {
        ...
        try {
            ...
            const buttons = new $.fn.dataTable.Buttons(table, {
                buttons: [
                    {
                        extend: 'excelHtml5',
                        title: 'Ventas_por_Producto_' + anioFiltro,
                        filename: 'Ventas_por_Producto_' + anioFiltro,
                        exportOptions: {
                            columns: [0, 1, 2, 3],
        ...
        try {
            ...
            const buttons2 = new $.fn.dataTable.Buttons(table2, {
                buttons: [
                    {
                        extend: 'excelHtml5',
                        title: 'Ventas_por_Producto_' + anioFiltro2,
                        filename: 'Ventas_por_Producto_' + anioFiltro2,
                        exportOptions: {
                            columns: [0, 1, 2, 3],
                            footer: false,
                            format: {
    ...
    $("#btnExportPdf").on('click', function (e) {
        ...
        exportarReporteTabla('dtVenta', 'pdf', 'Ventas_por_Producto', anioFiltro);
    });
    $("#btnExportPdf2").on('click', function (e) {
        e.preventDefault();
        const anioFiltro2 = (($("#mdvenselAnioSearch option:selected").val() || new Date().getFullYear()) - 1);
        exportarReporteTabla('dtVenta2', 'pdf', 'Ventas_por_Producto', anioFiltro2);
    });
    $("#btnExportAmbosExcel").on('click', function (e) {
        e.preventDefault();
        const anioFiltro = $("#mdvenselAnioSearch option:selected").val() || new Date().getFullYear();
        exportarReporteAmbosCuadros('dtVenta', 'dtVenta2', 'excel', 'Ventas_por_Producto', anioFiltro, anioFiltro - 1);
    });
    $("#btnExportAmbosPdf").on('click', function (e) {
        e.preventDefault();
        const anioFiltro = $("#mdvenselAnioSearch option:selected").val() || new Date().getFullYear();
        exportarReporteAmbosCuadros('dtVenta', 'dtVenta2', 'pdf', 'Ventas_por_Producto', anioFiltro, anioFiltro - 1);
    });
...
async function CargarTodo(dt, periodo) {
    ...
    let BusquedaCodAgencia = localStorage.getItem("lsventaproductoSel");
    ...
    if (AgenciaId > 0) {
        BusquedaCodAgencia = AgenciaId; // Si el usuario es agencia, se asigna su agencia por defecto
    }
    ...
    const listadoVentas = await getVentasProducto(BusquedaCodPais, BusquedaCancelacion, BusquedaAnio, BusquedaTipoReporte, BusquedaCodAgencia, BusquedaCodPromotor, BusquedaCodProducto);
...
async function getVentasProducto(int_pPaisId, int_pSituacionId, int_pAnio, int_TipoReporte, BusquedaCodAgencia, int_pPromotorId, BusquedaCodProducto) {
    const urlApiFecht = menuUrlApi + "Reportes/VentasProductoObtener";
    const urlParametro = "?int_pPaisId=" + int_pPaisId + "&int_pSituacionId=" + int_pSituacionId + "&int_pAnio=" + int_pAnio + "&int_TipoReporte=" + int_TipoReporte + "&int_pAgenciaId=" + BusquedaCodAgencia + "&int_pPromotorId=" + int_pPromotorId + "&int_pProductoId=" + BusquedaCodProducto;
        ...
        if (elcomboAgencia !== undefined) {
            ...
            if (cantElementos05 > 0) {
                ...
                $('#txtAgencia').on('typeahead:select', function (e, selection) {
                    ...
                    localStorage.setItem("lsventaproductoSel", selection.agenciaId); // Guarda el ID
                });
            }
        }
    }
...
$('#mdvenSelPaisSearch').change(async function () {
    localStorage.removeItem('lsventaproductoSel');
    let AgenciaId = 0;
    if (menuelOrigen !== 'U') {
        AgenciaId = menuelAgenciaUsuarioId;
    }
    ...
    if (AgenciaId === 0) {
        ...
        if (elcomboAgencia !== undefined && elcomboAgencia.length > 0) {
            ...
            $('#txtAgencia').off('typeahead:select').on('typeahead:select', function (e, selection) {
                ...
                localStorage.setItem("lsventaproductoSel", selection.agenciaId);
            });
        }
    }
    /*
