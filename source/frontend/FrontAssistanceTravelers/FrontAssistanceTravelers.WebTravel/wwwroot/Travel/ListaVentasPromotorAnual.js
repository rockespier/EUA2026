...
// Nueva función para actualizar los títulos de las tablas con el año
async function actualizarTitulosTablas() {
    ...
    // Actualizar título de la primera tabla (año actual o seleccionado)
    $('#anioTabla1').text(BusquedaAnio);

    // Actualizar título de la segunda tabla (año anterior)
    $('#anioTabla2').text(parseInt(BusquedaAnio) - 1);
}
...
async function drawTable(dt) {
    ...
    if (BusquedaTipoReporte == 2) {
        dTable.columns(1).header().to$().text('Cantidad');
    } else {
        dTable.columns(1).header().to$().text('Importe USD');
    }
...
function registrarEventosExportacion() {
        ...
        try {
            ...
            const buttons = new $.fn.dataTable.Buttons(table, {
                buttons: [
                    {
                        extend: 'excelHtml5',
                        title: 'Ventas_por_Promotor_' + anioFiltro,
                        filename: 'Ventas_por_Promotor_' + anioFiltro,
                        exportOptions: {
                            columns: [0, 1],
        ...
        try {
            ...
            const buttons2 = new $.fn.dataTable.Buttons(table2, {
                buttons: [
                    {
                        extend: 'excelHtml5',
                        title: 'Ventas_por_Promotor_' + anioFiltro2,
                        filename: 'Ventas_por_Promotor_' + anioFiltro2,
                        exportOptions: {
                            columns: [0, 1],
                            footer: false,
                            format: {
    ...
    $("#btnExportPdf").on('click', function (e) {
        ...
        exportarReporteTabla('dtVenta', 'pdf', 'Ventas_Anual_por_Promotor', anioFiltro);
    });
    $("#btnExportPdf2").on('click', function (e) {
        e.preventDefault();
        const anioFiltro2 = (($("#mdvenselAnioSearch option:selected").val() || new Date().getFullYear()) - 1);
        exportarReporteTabla('dtVenta2', 'pdf', 'Ventas_Anual_por_Promotor', anioFiltro2);
    });
    $("#btnExportAmbosExcel").on('click', function (e) {
        e.preventDefault();
        const anioFiltro = $("#mdvenselAnioSearch option:selected").val() || new Date().getFullYear();
        exportarReporteAmbosCuadros('dtVenta', 'dtVenta2', 'excel', 'Ventas_Anual_por_Promotor', anioFiltro, anioFiltro - 1);
    });
    $("#btnExportAmbosPdf").on('click', function (e) {
        e.preventDefault();
        const anioFiltro = $("#mdvenselAnioSearch option:selected").val() || new Date().getFullYear();
        exportarReporteAmbosCuadros('dtVenta', 'dtVenta2', 'pdf', 'Ventas_Anual_por_Promotor', anioFiltro, anioFiltro - 1);
    });
...
async function CargarTodo(dt, periodo) {
        ...
        footerCallback: function (row, data, start, end, display) {
            ...
            try {
                ...
                if (footerCells.every(cell => cell !== null && cell !== undefined)) {
                    ...
                    $(footerCells[1]).html(newCurrancy);
                }
            } catch (err) {
    ...
    const listadoVentas = await getVentasPromotor(BusquedaAnio, BusquedaCodPais, BusquedaTipoReporte, BusquedaCodPromotor);
...
async function getVentasPromotor(int_pAnio, int_pPaisId, int_TipoReporte, int_pPromotorId) {
    const urlApiFecht = menuUrlApi + "Reportes/VentasPromotorAnualObtener";
    const urlParametro = "?int_pAnio=" + int_pAnio + "&int_pPaisId=" + int_pPaisId + "&int_TipoReporte=" + int_TipoReporte + "&int_pUsuarioId=" + int_pPromotorId;
...
let elvalidarBuscar = $("#modalDatosVentaSearch").validate({
    rules: {
        mdvenselPromotorSearch: "required"
    },
    messages: {
        mdvenselPromotorSearch: "Seleccione un promotor."
    },
    errorElement: "em",
