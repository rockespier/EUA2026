...
async function cargarAcciones() {
    ...
    setTimeout(async () => {
        ...
        // Actualizar títulos con año y mes
        await actualizarTitulosTablas();
...
// Nueva función para actualizar los títulos de las tablas con año y mes
async function actualizarTitulosTablas() {
    ...
    if (BusquedaMes === "") {
        BusquedaMes = mesActual;
    }
    
    // Actualizar título de la primera tabla (año/mes actual o seleccionado)
    $('#anioMesTabla1').text(BusquedaAnio + '/' + BusquedaMes);
    
    // Calcular el mes/año anterior
    let anioAnterior = parseInt(BusquedaAnio);
    let mesAnterior = parseInt(BusquedaMes) - 1;
    ...
    // Actualizar título de la segunda tabla (mes/año anterior)
    $('#anioMesTabla2').text(anioAnterior + '/' + mesAnterior);
}
...
async function CargarTodo(dt, periodo) {
    ...
    const listadoVentas = await getVentasPromotor(BusquedaAnio, BusquedaCodPais, BusquedaTipoReporte, BusquedaCodPromotor, BusquedaMes);
...
async function getVentasPromotor(int_pAnio, int_pPaisId, int_TipoReporte, int_pPromotorId, int_pMes) {
    const urlApiFecht = menuUrlApi + "Reportes/VentasPromotorDiarioObtener";
    const urlParametro = "?int_pAnio=" + int_pAnio + "&int_pPaisId=" + int_pPaisId + "&int_TipoReporte=" + int_TipoReporte + "&int_pUsuarioId=" + int_pPromotorId + "&int_pMes=" + int_pMes;
...
async function clickValidarBuscar() {
    if ($("#modalDatosVentaSearch").valid()) {
        showLoader(); // Mostrar loader durante la carga
...
async function cargarCombosBusqueda() {
    ...
    const elcomboTipo = await getPromotoresPais(menuUserId, 0);
