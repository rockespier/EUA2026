...
function registrarEventosExportacion() {
        ...
        try {
            ...
            const buttons2 = new $.fn.dataTable.Buttons(table2, {
                buttons: [
                    {
                        extend: 'excelHtml5',
                        title: 'Ventas_por_rango_edad_' + anioFiltro2,
                        filename: 'Ventas_por_rango_edad_' + anioFiltro2,
                        exportOptions: {
                            columns: [0, 1, 2, 3],
                            footer: false,
                            format: {
    ...
    $("#btnExportPdf").on('click', function (e) {
        ...
        exportarReporteTabla('dtVenta', 'pdf', 'Ventas_por_rango_edad', anioFiltro);
    });
    $("#btnExportPdf2").on('click', function (e) {
        e.preventDefault();
        const anioFiltro2 = (($("#mdvenselAnioSearch option:selected").val() || new Date().getFullYear()) - 1);
        exportarReporteTabla('dtVenta2', 'pdf', 'Ventas_por_rango_edad', anioFiltro2);
    });
    $("#btnExportAmbosExcel").on('click', function (e) {
        e.preventDefault();
        const anioFiltro = $("#mdvenselAnioSearch option:selected").val() || new Date().getFullYear();
        exportarReporteAmbosCuadros('dtVenta', 'dtVenta2', 'excel', 'Ventas_por_rango_edad', anioFiltro, anioFiltro - 1);
    });
    $("#btnExportAmbosPdf").on('click', function (e) {
        e.preventDefault();
        const anioFiltro = $("#mdvenselAnioSearch option:selected").val() || new Date().getFullYear();
        exportarReporteAmbosCuadros('dtVenta', 'dtVenta2', 'pdf', 'Ventas_por_rango_edad', anioFiltro, anioFiltro - 1);
    });
...
async function CargarTodo(dt, periodo) {
    ...
    const listadoVentas = await getVentasRangoEdad(BusquedaCodPais, BusquedaCancelacion, BusquedaAnio, BusquedaTipoReporte, BusquedaCodAgencia, BusquedaCodPromotor, BusquedaCodProducto);
